...
# =========================================================
# CONSTANTES E PARÂMETROS DO MOTOR FLOW
# =========================================================

TEMPO_ALVO_BASE = {1: 45, 2: 90, 3: 150, 4: 240, 5: 360}

MULT_TEMPO_DISCIPLINA = {
    "Língua Portuguesa": 1.5,
    "Língua Inglesa": 1.5,
    "Raciocínio Lógico": 1.3,
    "Matemática e Estatística Aplicada": 1.3,
}
...
DIFICULDADE_R = {1: 10, 2: 30, 3: 50, 4: 70, 5: 90}
ESCALA_ELO = 40.0    
K_BASE = 14.0        
ALVO_ESPERADO = 0.75 
...
K_MULT = {
    ...
    (True,  "alvo"):        1.0,
    (True,  "lento"):       0.6,
    (True,  "muito_lento"): 0.3,   
    (False, "rapido"):      0.6,
    (False, "alvo"):        1.0,
    (False, "lento"):       1.4,
    (False, "muito_lento"): 1.4,
}

JANELA = 8
ALVO_ACERTO_MIN = 0.60
ALVO_ACERTO_MAX = 0.85
QUESTOES_CALIBRACAO = 6
OFFSETS_REVISAO = (3, 8, 20)
R_INICIAL = 50.0

# =========================================================
# CORE DO MOTOR FLOW (LÓGICA PURA)
# =========================================================

def tempo_alvo_seg(nivel, disciplina=None):
    base = TEMPO_ALVO_BASE.get(int(nivel), 150)
    mult = MULT_TEMPO_DISCIPLINA.get(disciplina, 1.0)
...
def prob_esperada(r, nivel):
    dif = DIFICULDADE_R[int(nivel)]
    return 1.0 / (1.0 + 10 ** ((dif - float(r)) / ESCALA_ELO))

def nivel_de_flow(r):
    return min(DIFICULDADE_R, key=lambda n: abs(prob_esperada(r, n) - ALVO_ESPERADO))

def classificar_tempo(tempo_seg, alvo_seg):
    if alvo_seg <= 0: return "alvo"
    razao = tempo_seg / float(alvo_seg)
    if razao > 2.0: return "muito_lento"
    if razao > 1.3: return "lento"
    if razao < 0.7: return "rapido"
    return "alvo"

def calcular_delta(acertou, tempo_seg, alvo_seg, r_atual=R_INICIAL, nivel=3, sensacao=None, q_global=999):
    faixa = classificar_tempo(tempo_seg, alvo_seg)
    esperado = prob_esperada(r_atual, nivel)
    resultado = 1.0 if acertou else 0.0
    k = K_BASE * K_MULT[(bool(acertou), faixa)]
    delta = k * (resultado - esperado)
    if sensacao == "chutei" and acertou and delta > 0: delta = delta / 2.0
    if q_global <= QUESTOES_CALIBRACAO: delta = delta * 2.0
    return round(delta, 2), faixa, round(esperado, 3)

def atualizar_estado(estado, acertou, delta):
    r_antes = float(estado.get("r", R_INICIAL))
    r_depois = max(0.0, min(100.0, r_antes + delta))
    if acertou:
        streak_a = int(estado.get("streak_acertos", 0)) + 1
        streak_e = 0
    else:
        streak_a = 0
        streak_e = int(estado.get("streak_erros", 0)) + 1
    return {
        "r": round(r_depois, 2),
        "streak_acertos": streak_a,
        "streak_erros": streak_e,
        "total_respondidas": int(estado.get("total_respondidas", 0)) + 1,
        "_r_antes": round(r_antes, 2),
    }

def aproveitamento_janela(historico_acertos, n=JANELA):
    janela = list(historico_acertos)[-n:]
    if not janela: return None
    return sum(janela) / float(len(janela))

def ajuste_nivel_global(historico_acertos):
    acc = aproveitamento_janela(historico_acertos)
    if acc is None or len(historico_acertos) < JANELA: return 0
    if acc > ALVO_ACERTO_MAX: return 1
    if acc < ALVO_ACERTO_MIN: return -1
    return 0

def escolher_topico(estados, topicos_por_disciplina, historico_topicos=(), revisoes_pendentes=(), forcar_ancora=False, rng=None):
    rng = rng or random
    universo = []
    for disc, topicos in topicos_por_disciplina.items():
        if disc not in PESOS_DISCIPLINA: continue
        for t in topicos: universo.append((disc, t))
    if not universo: return None, None, "sem_topicos"
    ultimas_disciplinas = list(historico_topicos)[-2:]
    def penaliza_repeticao(candidatos):
        frescos = [c for c in candidatos if c[0] not in ultimas_disciplinas]
        return frescos if frescos else candidatos
    if forcar_ancora:
        respondidos = [(d, t) for (d, t) in universo if estados.get(t, {}).get("total_respondidas", 0) > 0]
        if respondidos:
            melhor = max(respondidos, key=lambda c: estados.get(c[1], {}).get("r", R_INICIAL))
            return melhor[0], melhor[1], "ancora"
    pendentes = [(d, t) for (d, t) in universo if t in set(revisoes_pendentes)]
    if pendentes:
        escolha = rng.choice(penaliza_repeticao(pendentes))
        return escolha[0], escolha[1], "revisao_espacada"
    novos = [(d, t) for (d, t) in universo if estados.get(t, {}).get("total_respondidas", 0) == 0]
    if novos:
        candidatos = penaliza_repeticao(novos)
        pesos = [PESOS_DISCIPLINA.get(d, 1) for (d, _) in candidatos]
        escolha = rng.choices(candidatos, weights=pesos, k=1)[0]
        return escolha[0], escolha[1], "inedito"
    candidatos = penaliza_repeticao(universo)
    candidatos = sorted(candidatos, key=lambda c: estados.get(c[1], {}).get("r", R_INICIAL))[:8]
    pesos = [max(1.0, 101.0 - estados.get(t, {}).get("r", R_INICIAL)) * PESOS_DISCIPLINA.get(d, 1) / 10.0 for (d, t) in candidatos]
    escolha = rng.choices(candidatos, weights=pesos, k=1)[0]
    return escolha[0], escolha[1], "ponto_fraco"

def escolher_nivel(estado_topico, ajuste_global=0, sensacao_anterior=None):
    nivel = nivel_de_flow(estado_topico.get("r", R_INICIAL))
    nivel_ultimo = int(estado_topico.get("nivel_ultimo") or nivel)
    if int(estado_topico.get("streak_acertos", 0)) >= 3 and nivel <= nivel_ultimo: nivel = nivel_ultimo + 1
    if int(estado_topico.get("streak_erros", 0)) >= 2 and nivel >= nivel_ultimo: nivel = nivel_ultimo - 1
    nivel += int(ajuste_global)
    if sensacao_anterior == "tedio": nivel += 1
    elif sensacao_anterior == "ansiedade": nivel -= 1
    return max(1, min(5, nivel))

def selecionar_questao(banco, topico, nivel, ids_ja_respondidos=(), disciplina=None, rng=None):
    rng = rng or random
    ja = set(ids_ja_respondidos)
    def filtra(criterio, inedita=True):
        pool = [q for q in banco if criterio(q)]
        if inedita: pool = [q for q in pool if q["id"] not in ja]
        return pool
    tentativas = [
        (lambda q: q["topico_edital"] == topico and int(q["nivel"]) == nivel, "exata"),
        (lambda q: q["topico_edital"] == topico and abs(int(q["nivel"]) - nivel) == 1, "nivel_vizinho"),
        (lambda q: q["topico_edital"] == topico, "qualquer_nivel_do_topico"),
        (lambda q: q.get("disciplina") == disciplina and int(q["nivel"]) == nivel, "outro_topico_mesma_disciplina"),
        (lambda q: q.get("disciplina") == disciplina, "qualquer_da_disciplina"),
    ]
    for criterio, motivo in tentativas:
        pool = filtra(criterio)
        if pool: return rng.choice(pool), motivo
    pool = [q for q in banco if q["topico_edital"] == topico]
    if pool: return rng.choice(pool), "repeticao"
    return None, "banco_vazio"

def agendar_revisoes(topico, disciplina, q_global):
    return [{"topico_edital": topico, "disciplina": disciplina, "due_q": int(q_global) + off, "cumprida": False} for off in OFFSETS_REVISAO]

CAMPOS_OBRIGATORIOS = ("disciplina", "topico_edital", "nivel", "enunciado", "alternativas", "gabarito")

def hash_questao(enunciado, gabarito):
    base = re.sub(r"\s+", " ", str(enunciado)).strip().lower() + "|" + str(gabarito).strip().upper()
    return hashlib.sha1(base.encode("utf-8")).hexdigest()

def normalizar_topico(disciplina, topico_bruto, topicos_edital):
    oficiais = topicos_edital.get(disciplina, [])
    if not oficiais: return None, False
    alvo = str(topico_bruto or "").strip()
    if alvo in oficiais: return alvo, False
    m = re.match(r"^\s*([IVX]+\.\d+|\d+\.\d+|\d+)", alvo)
    if m:
        codigo = m.group(1)
        for of in oficiais:
            if re.match(r"^\s*" + re.escape(codigo) + r"(\s|\.|$)", of): return of, True
    parecidos = difflib.get_close_matches(alvo, oficiais, n=1, cutoff=0.45)
    if parecidos: return parecidos[0], True
    return None, False

def validar_questao(bruta, topicos_edital, disciplinas_validas):
    avisos = []
    faltando = [c for c in CAMPOS_OBRIGATORIOS if c not in bruta or bruta[c] in (None, "")]
    if faltando: return None, ["campos ausentes: " + ", ".join(faltando)]
    disciplina = str(bruta["disciplina"]).strip()
    if disciplina not in disciplinas_validas:
        parecidos = difflib.get_close_matches(disciplina, list(disciplinas_validas), n=1, cutoff=0.5)
        if not parecidos: return None, [f"disciplina desconhecida: '{disciplina}'"]
        avisos.append(f"disciplina reancorada: '{disciplina}' -> '{parecidos[0]}'")
        disciplina = parecidos[0]
    topico, ajustado = normalizar_topico(disciplina, bruta["topico_edital"], topicos_edital)
    if topico is None: return None, [f"tópico não encontrado no edital: '{bruta['topico_edital']}'"]
    if ajustado: avisos.append(f"tópico reancorado -> '{topico[:45]}...'")
    try: nivel = int(bruta["nivel"])
    except (TypeError, ValueError): return None, [f"nível inválido: {bruta['nivel']}"]
    if not 1 <= nivel <= 5: return None, [f"nível fora de 1-5: {nivel}"]
    alts = bruta["alternativas"]
    if isinstance(alts, list): alts = {letra: txt for letra, txt in zip("ABCDE", alts)}
    if not isinstance(alts, dict): return None, ["alternativas devem ser objeto {'A': ...} ou lista"]
    alts = {str(k).strip().upper()[:1]: str(v) for k, v in alts.items()}
    if set(alts.keys()) != set("ABCDE"): return None, [f"precisa ter exatamente 5 alternativas A-E (veio: {sorted(alts.keys())})"]
    gabarito = str(bruta["gabarito"]).strip().upper()[:1]
    if gabarito not in "ABCDE": return None, [f"gabarito inválido: '{bruta['gabarito']}'"]
    questao = {
        "disciplina": disciplina, "topico_edital": topico, "nivel": nivel, "formato": str(bruta.get("formato", "direta")),
        "enunciado": str(bruta["enunciado"]).strip(), "alternativas": alts, "gabarito": gabarito,
        "comentario": str(bruta.get("comentario", "") or ""), "distratores": bruta.get("distratores") if isinstance(bruta.get("distratores"), dict) else {},
        "pegadinha": str(bruta.get("pegadinha", "") or ""), "fonte": str(bruta.get("fonte", "IA (Estilo FGV)")), "conferir_lei": bool(bruta.get("conferir_lei", False)),
    }
    questao["hash"] = hash_questao(questao["enunciado"], questao["gabarito"])
    if not questao["comentario"]: avisos.append("sem comentário — você vai errar de novo sem saber por quê")
    return questao, avisos

def validar_lote(lista_bruta, topicos_edital, disciplinas_validas):
    validas, rejeitadas, avisos = [], [], []
    vistos = set()
    for i, bruta in enumerate(lista_bruta):
        if not isinstance(bruta, dict):
            rejeitadas.append({"indice": i, "erros": ["item não é um objeto JSON"]})
            continue
        q, msgs = validar_questao(bruta, topicos_edital, disciplinas_validas)
        if q is None:
            rejeitadas.append({"indice": i, "erros": msgs, "preview": str(bruta.get("enunciado", ""))[:80]})
            continue
        if q["hash"] in vistos:
            rejeitadas.append({"indice": i, "erros": ["duplicada dentro do lote"], "preview": q["enunciado"][:80]})
            continue
        vistos.add(q["hash"])
        validas.append(q)
        for m in msgs: avisos.append(f"#{i + 1}: {m}")
    return validas, rejeitadas, avisos
...
# --- INTEGRAÇÃO BANCO DE DADOS: MODO FLOW ---
@st.cache_data(ttl=120, show_spinner=False)
def carregar_banco(_sb, _versao=0):
    resp = _sb.table("questoes").select("*").execute()
    banco = resp.data or []
    for q in banco:
        if isinstance(q.get("alternativas"), str):
            try: q["alternativas"] = json.loads(q["alternativas"])
            except Exception: q["alternativas"] = {}
        if isinstance(q.get("distratores"), str):
            try: q["distratores"] = json.loads(q["distratores"])
            except Exception: q["distratores"] = {}
    return banco

def carregar_estados(sb):
    resp = sb.table("flow_estado").select("*").execute()
    return {
        linha["topico_edital"]: {
            "r": float(linha.get("r", R_INICIAL)),
            "disciplina": linha.get("disciplina"),
            "streak_acertos": int(linha.get("streak_acertos", 0)),
            "streak_erros": int(linha.get("streak_erros", 0)),
            "total_respondidas": int(linha.get("total_respondidas", 0)),
            "nivel_ultimo": int(linha.get("nivel_ultimo") or 3),
        }
        for linha in (resp.data or [])
    }

def carregar_respostas(sb, limite=400):
    resp = (sb.table("flow_respostas").select("*")
            .order("respondido_em", desc=True).limit(limite).execute())
    dados = list(reversed(resp.data or []))
    return pd.DataFrame(dados) if dados else pd.DataFrame()

def contar_q_global(sb):
    resp = sb.table("flow_respostas").select("id", count="exact").limit(1).execute()
    return int(resp.count or 0)

def revisoes_vencidas(sb, q_global):
    resp = (sb.table("flow_revisao").select("topico_edital")
            .eq("cumprida", False).lte("due_q", q_global).execute())
    return [linha["topico_edital"] for linha in (resp.data or [])]

def sortear_proxima(sb, TOPICOS_EDITAL):
    banco = carregar_banco(sb, st.session_state.get("flow_banco_versao", 0))
    if not banco:
        st.session_state.flow_questao = None
        st.session_state.flow_erro_sorteio = "banco_vazio"
        return
    estados = carregar_estados(sb)
    q_global = contar_q_global(sb)
    pendentes = revisoes_vencidas(sb, q_global)
    hist_acertos = st.session_state.get("flow_hist_acertos", [])
    hist_disc = st.session_state.get("flow_hist_disciplinas", [])
    sensacao_ant = st.session_state.get("flow_ultima_sensacao")

    erros_seguidos = 0
    for a in reversed(hist_acertos):
        if a == 0: erros_seguidos += 1
        else: break
    forcar_ancora = erros_seguidos >= 2 or sensacao_ant == "ansiedade"

    disciplina, topico, motivo = escolher_topico(
        estados, TOPICOS_EDITAL, historico_topicos=hist_disc,
        revisoes_pendentes=pendentes, forcar_ancora=forcar_ancora,
    )
    if topico is None:
        st.session_state.flow_questao = None
        st.session_state.flow_erro_sorteio = "sem_topicos"
        return

    estado_topico = estados.get(topico, {"r": R_INICIAL, "streak_acertos": 0, "streak_erros": 0, "total_respondidas": 0, "nivel_ultimo": 3})
    ajuste = ajuste_nivel_global(hist_acertos)

    if motivo == "ancora": nivel = max(1, min(2, nivel_de_flow(estado_topico["r"]) - 1))
    else: nivel = escolher_nivel(estado_topico, ajuste, sensacao_ant)

    respondidas = st.session_state.get("flow_ids_respondidos", set())
    questao, fallback = selecionar_questao(banco, topico, nivel, ids_ja_respondidos=respondidas, disciplina=disciplina)
    if questao is None:
        st.session_state.flow_questao = None
        st.session_state.flow_erro_sorteio = f"sem_questao:{topico}"
        return

    st.session_state.flow_questao = questao
    st.session_state.flow_meta = {
        "topico_alvo": topico, "disciplina_alvo": disciplina, "nivel_alvo": nivel,
        "nivel_servido": int(questao["nivel"]), "motivo": motivo, "fallback": fallback,
        "ajuste_global": ajuste, "r_topico": estado_topico["r"],
        "tempo_alvo": tempo_alvo_seg(questao["nivel"], questao.get("disciplina")), "q_global": q_global + 1,
    }
    st.session_state.flow_inicio = time.time()
    st.session_state.flow_feedback = None
    st.session_state.flow_erro_sorteio = None

def registrar_resposta(sb, resposta, sensacao, tempo_seg):
    questao = st.session_state.flow_questao
    meta = st.session_state.flow_meta
    topico = questao["topico_edital"]
    disciplina = questao.get("disciplina")
    acertou = (resposta == questao["gabarito"])
    alvo = meta["tempo_alvo"]
    q_global = meta["q_global"]

    estados = carregar_estados(sb)
    estado_pre = estados.get(topico, {"r": R_INICIAL, "streak_acertos": 0, "streak_erros": 0, "total_respondidas": 0, "nivel_ultimo": 3})
    r_antes_calc = estado_pre.get("r", R_INICIAL)
    delta, faixa, esperado = calcular_delta(acertou, tempo_seg, alvo, r_antes_calc, int(questao["nivel"]), sensacao, q_global)
    novo = atualizar_estado(estado_pre, acertou, delta)

    sb.table("flow_estado").upsert({
        "topico_edital": topico, "disciplina": disciplina, "r": novo["r"],
        "streak_acertos": novo["streak_acertos"], "streak_erros": novo["streak_erros"],
        "total_respondidas": novo["total_respondidas"], "nivel_ultimo": int(questao["nivel"]),
        "atualizado_em": datetime.utcnow().isoformat(),
    }, on_conflict="topico_edital").execute()

    sb.table("flow_respostas").insert({
        "sessao_id": st.session_state.flow_sessao_id, "questao_id": questao["id"],
        "disciplina": disciplina, "topico_edital": topico, "nivel": int(questao["nivel"]),
        "resposta": resposta, "acertou": acertou, "tempo_seg": int(tempo_seg),
        "tempo_alvo_seg": int(alvo), "faixa_tempo": faixa, "sensacao": sensacao,
        "r_antes": novo["_r_antes"], "r_depois": novo["r"], "delta_r": delta, "q_global": q_global,
    }).execute()

    if not acertou:
        sb.table("flow_revisao").insert(agendar_revisoes(topico, disciplina, q_global)).execute()

    (sb.table("flow_revisao").update({"cumprida": True})
     .eq("topico_edital", topico).eq("cumprida", False).lte("due_q", q_global).execute())

    st.session_state.flow_hist_acertos = st.session_state.get("flow_hist_acertos", []) + [1 if acertou else 0]
    st.session_state.flow_hist_disciplinas = st.session_state.get("flow_hist_disciplinas", []) + [disciplina]
    st.session_state.flow_ultima_sensacao = sensacao
    st.session_state.flow_ids_respondidos = st.session_state.get("flow_ids_respondidos", set()) | {questao["id"]}
    st.session_state.flow_sessao_log = st.session_state.get("flow_sessao_log", []) + [{
        "disciplina": disciplina, "topico_edital": topico, "acertou": acertou, "tempo_seg": int(tempo_seg),
    }]

    st.session_state.flow_feedback = {
        "acertou": acertou, "resposta": resposta, "gabarito": questao["gabarito"],
        "tempo": int(tempo_seg), "alvo": alvo, "faixa": faixa, "delta": delta,
        "r_antes": novo["_r_antes"], "r_depois": novo["r"], "esperado": esperado,
        "nivel_antes": nivel_por_r(novo["_r_antes"]), "nivel_depois": nivel_por_r(novo["r"]), "questao": questao,
    }

def fechar_sessao(sb):
    log = st.session_state.get("flow_sessao_log", [])
    if not log: return 0
    df = pd.DataFrame(log)
    agora = datetime.utcnow() - timedelta(hours=3)
    linhas = []
    for disciplina, grupo in df.groupby("disciplina"):
        certas = int(grupo["acertou"].sum())
        erradas = int(len(grupo) - certas)
        minutos = int(round(grupo["tempo_seg"].sum() / 60.0))
        topicos = ", ".join(sorted(grupo["topico_edital"].unique()))
        linhas.append({
            "data": str(agora.date()), "horario": agora.strftime("%H:%M:%S"),
            "grupo_muscular": "Estudos", "exercicio": disciplina,
            "series": 0, "repeticoes": certas + erradas, "carga_kg": 0.0, "descanso_seg": 0, "duracao_min": minutos,
            "distancia_km": 0.0, "alimentacao_saudavel": "", "alimentacao_besteirol": "", "peso_corporal": 0.0,
            "dados_extras": {
                "topico_edital": topicos, "q_certas": certas, "q_erradas": erradas,
                "tempo_video": 0, "fonte_questoes": "IA (Estilo FGV)", "origem": "modo_flow",
            },
        })
    sb.table("treinos").insert(linhas).execute()
    st.session_state.flow_sessao_log = []
    st.session_state.flow_sessao_id = str(uuid.uuid4())[:8]
    return len(linhas)

def _timer_visual(segundos_ja_corridos, tempo_alvo):
    html = """
    <!DOCTYPE html><html><head><style>
      body { background: transparent; font-family: sans-serif; text-align: center; margin: 0; }
      .t { font-size: 40px; font-weight: bold; color: #009CA6; text-shadow: 0 0 12px rgba(0,156,166,.4); }
      .t.estourou { color: #F43F5E; text-shadow: 0 0 12px rgba(244,63,94,.5); }
      .lbl { color: #777; font-size: 12px; letter-spacing: 1px; }
    </style></head><body>
      <div class="t" id="t">00:00</div>
      <div class="lbl" id="l">ALVO [ALVO_FMT]</div>
      <script>
        var s = [INICIO]; var alvo = [ALVO];
        var el = document.getElementById('t'); var lb = document.getElementById('l');
        function fmt(x){var m=Math.floor(x/60),r=x%60;return (m<10?'0':'')+m+':'+(r<10?'0':'')+r;}
        function tick(){ el.innerText = fmt(s);
          if (s > alvo * 1.3) { el.classList.add('estourou'); lb.innerText = 'ACIMA DO ALVO — vale mais chutar e seguir'; }
          s++; }
        tick(); setInterval(tick, 1000);
      </script>
    </body></html>
    """
    alvo_fmt = f"{tempo_alvo // 60:02d}:{tempo_alvo % 60:02d}"
    html = (html.replace("[INICIO]", str(int(segundos_ja_corridos)))
                .replace("[ALVO]", str(int(tempo_alvo)))
                .replace("[ALVO_FMT]", alvo_fmt))
    components.html(html, height=90)

MOTIVOS = {
    "ancora": ("🛟", "Âncora de confiança", "#10B981"),
    "revisao_espacada": ("🔁", "Revisão espaçada (você errou isso antes)", "#F59E0B"),
    "inedito": ("🆕", "Tópico inédito", "#8B5CF6"),
    "ponto_fraco": ("🎯", "Ponto fraco", "#F43F5E"),
}

def _sub_praticar(sb, TOPICOS_EDITAL):
    if "flow_sessao_id" not in st.session_state: st.session_state.flow_sessao_id = str(uuid.uuid4())[:8]
    if "flow_ids_respondidos" not in st.session_state: st.session_state.flow_ids_respondidos = set()

    hist = st.session_state.get("flow_hist_acertos", [])
    acc = aproveitamento_janela(hist)

    if acc is None: estado_txt, cor = "Calibrando…", "#8B5CF6"
    elif acc > ALVO_ACERTO_MAX: estado_txt, cor = "😴 FÁCIL DEMAIS — subindo o nível", "#F59E0B"
    elif acc < ALVO_ACERTO_MIN: estado_txt, cor = "😰 DIFÍCIL DEMAIS — baixando o nível", "#F43F5E"
    else: estado_txt, cor = "🌊 EM FLOW", "#10B981"

    acc_fmt = f"{acc * 100:.0f}%" if acc is not None else "—"
    st.markdown(
        f'<div style="background:rgba(0,0,0,.3);border:1px solid {cor};border-left:5px solid {cor};'
        f'padding:14px 18px;border-radius:10px;margin-bottom:18px;display:flex;'
        f'justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;">'
        f'<span style="color:{cor};font-weight:800;font-size:18px;">{estado_txt}</span>'
        f'<span style="color:#AAA;font-size:13px;">Janela({JANELA}): <b style="color:#FFF;">{acc_fmt}</b>'
        f' &nbsp;·&nbsp; alvo 70–80% &nbsp;·&nbsp; sessão: <b style="color:#FFF;">{len(hist)}</b> questões</span>'
        f'</div>', unsafe_allow_html=True)

    feedback = st.session_state.get("flow_feedback")
    questao = st.session_state.get("flow_questao")

    if feedback:
        f = feedback; q = f["questao"]
        cor_f = "#10B981" if f["acertou"] else "#F43F5E"
        titulo = "✅ CORRETO" if f["acertou"] else "❌ ERRADO"
        seta = "▲" if f["delta"] > 0 else ("▼" if f["delta"] < 0 else "=")
        mudou_nivel = ""
        if f["nivel_depois"] != f["nivel_antes"]: mudou_nivel = f' &nbsp;<b style="color:#8B5CF6;">N{f["nivel_antes"]}→N{f["nivel_depois"]}</b>'

        st.markdown(
            f'<div style="background:rgba(0,0,0,.35);border:2px solid {cor_f};padding:20px;'
            f'border-radius:10px;margin-bottom:16px;">'
            f'<span style="color:{cor_f};font-size:22px;font-weight:900;">{titulo}</span>'
            f'<span style="color:#AAA;font-size:14px;"> — gabarito <b style="color:#FFF;">{f["gabarito"]}</b>'
            f' (você: {f["resposta"]}) · {f["tempo"]}s vs alvo {f["alvo"]}s → <b>{f["faixa"]}</b></span><br>'
            f'<span style="color:#DDD;font-size:14px;margin-top:8px;display:inline-block;">'
            f'R do tópico: {f["r_antes"]:.0f} {seta} <b style="color:{cor_f};">{f["r_depois"]:.0f}</b>'
            f' ({f["delta"]:+.1f}){mudou_nivel}</span><br>'
            f'<span style="color:#666;font-size:12px;">o motor estimava '
            f'{f.get("esperado", 0) * 100:.0f}% de chance de você acertar essa — '
            f'quanto mais longe do esperado, mais o R se move</span></div>', unsafe_allow_html=True)

        if q.get("comentario"): st.markdown(f"**💡 Por que {f['gabarito']}:** {q['comentario']}")
        distratores = q.get("distratores") or {}
        if distratores:
            linhas = " · ".join(f"**({k})** {v}" for k, v in sorted(distratores.items()) if k != f["gabarito"])
            st.markdown(f"**Por que não:** {linhas}")
        if q.get("pegadinha"): st.info(f"📌 **Pegadinha FGV:** {q['pegadinha']}")
        if q.get("conferir_lei"): st.warning("⚠️ Questão de legislação gerada por IA — confira o dispositivo na letra da lei.")

        if st.button("➡️ Próxima questão", use_container_width=True, type="primary"):
            sortear_proxima(sb, TOPICOS_EDITAL)
            st.rerun()

        st.write("---")
        c1, c2 = st.columns(2)
        with c1:
            if st.button("💾 Fechar sessão e computar no Dashboard", use_container_width=True):
                n = fechar_sessao(sb)
                st.success(f"{n} registro(s) gravados em `treinos`. A meta diária já contabilizou.")
        with c2:
            if st.button("🔄 Zerar contadores da sessão", use_container_width=True):
                for k in ("flow_hist_acertos", "flow_hist_disciplinas", "flow_sessao_log"): st.session_state[k] = []
                st.rerun()
        return

    if not questao:
        erro = st.session_state.get("flow_erro_sorteio")
        if erro == "banco_vazio": st.warning("📭 O banco de questões está vazio. Vá na aba **📥 Importar**.")
        elif erro and erro.startswith("sem_questao"):
            st.warning(f"Nenhuma questão disponível para o tópico sorteado ({erro.split(':', 1)[1][:60]}…). Importe mais.")
        if st.button("🎲 Sortear questão", use_container_width=True, type="primary"):
            sortear_proxima(sb, TOPICOS_EDITAL)
            st.rerun()
        return

    meta = st.session_state.flow_meta
    icone, label, cor_m = MOTIVOS.get(meta["motivo"], ("•", meta["motivo"], "#009CA6"))

    st.markdown(
        f'<div style="border-top:1px solid #1F1F1F;border-bottom:1px solid #1F1F1F;'
        f'padding:10px 0;margin-bottom:16px;display:flex;justify-content:space-between;'
        f'flex-wrap:wrap;gap:8px;font-size:13px;">'
        f'<span style="color:#009CA6;font-weight:700;">Q{meta["q_global"]} · '
        f'<span style="color:#FFF;">N{questao["nivel"]}</span> · {questao["disciplina"]}</span>'
        f'<span style="color:{cor_m};">{icone} {label} · R={meta["r_topico"]:.0f}</span></div>'
        f'<div style="color:#888;font-size:12px;margin-bottom:14px;">📖 {questao["topico_edital"]}</div>',
        unsafe_allow_html=True)

    decorrido = time.time() - st.session_state.get("flow_inicio", time.time())
    _timer_visual(decorrido, meta["tempo_alvo"])

    with st.container(border=True): st.markdown(questao["enunciado"])

    alts = questao["alternativas"]
    with st.form(f"flow_resposta_{questao['id']}", clear_on_submit=True):
        resposta = st.radio("Sua resposta:", options=["A", "B", "C", "D", "E"], format_func=lambda k: f"({k})  {alts.get(k, '')}", index=None)
        c1, c2 = st.columns([1, 1])
        with c1:
            sensacao = st.selectbox("Como foi?", ["ok", "chutei", "tedio", "ansiedade", "travei"], format_func=lambda s: {"ok": "🙂 Normal", "chutei": "🎲 Chutei", "tedio": "😴 Tédio (fácil demais)", "ansiedade": "😰 Ansiedade (difícil demais)", "travei": "🧱 Travei"}[s])
        with c2:
            ajuste_manual = st.number_input("Corrigir tempo (min) — só se você se distraiu", min_value=0.0, value=0.0, step=0.5, help="0 = usar o tempo medido automaticamente.")
        enviar = st.form_submit_button("✅ Responder", use_container_width=True, type="primary")

    if enviar:
        if resposta is None: st.error("Marque uma alternativa antes de enviar.")
        else:
            tempo = int(ajuste_manual * 60) if ajuste_manual > 0 else int(time.time() - st.session_state.flow_inicio)
            tempo = max(3, tempo)
            registrar_resposta(sb, resposta, sensacao, tempo)
            st.rerun()

def _sub_importar(sb, TOPICOS_EDITAL, DISCIPLINAS_ESTUDO):
    st.markdown("#### 📥 Importar questões geradas pela IA")
    with st.expander("📋 Gerar o pedido para a IA (baseado nos seus pontos fracos de hoje)"):
        estados = carregar_estados(sb)
        alvos = []
        for disc, topicos in TOPICOS_EDITAL.items():
            if disc not in PESOS_DISCIPLINA: continue
            for t in topicos:
                e = estados.get(t)
                r = e["r"] if e else R_INICIAL
                vistas = e["total_respondidas"] if e else 0
                alvos.append((r, vistas, disc, t))
        alvos.sort(key=lambda x: (x[0], -x[1]))
        selecao = alvos[:6]

        linhas = "\n".join(f'- disciplina: "{d}" | topico_edital: "{t}" | nivel: {nivel_por_r(r)} | 4 questões' for r, _, d, t in selecao)
        pedido = (
            "Gere questões inéditas no estilo FGV (5 alternativas A-E, enunciado com caso concreto, "
            "distratores diagnósticos) para os alvos abaixo.\n\n"
            f"{linhas}\n\n"
            "Devolva APENAS um array JSON válido, sem texto antes ou depois, com objetos neste formato:\n"
            '[{"disciplina":"...","topico_edital":"...","nivel":3,"formato":"direta",'
            '"enunciado":"...","alternativas":{"A":"...","B":"...","C":"...","D":"...","E":"..."},'
            '"gabarito":"C","comentario":"por que a correta está certa",'
            '"distratores":{"A":"erro que essa alternativa representa","B":"...","D":"...","E":"..."},'
            '"pegadinha":"o que a banca testa de fato","conferir_lei":false}]\n\n'
            'Use os valores de "disciplina" e "topico_edital" EXATAMENTE como escritos acima. '
            'Em questões de legislação, marque "conferir_lei": true.'
        )
        st.code(pedido, language="text")

    st.write("---")
    modo = st.radio("Origem:", ["📋 Colar JSON", "📎 Enviar arquivo .json"], horizontal=True)
    bruto = None

    if modo == "📋 Colar JSON":
        texto = st.text_area("Cole aqui o array JSON:", height=220, placeholder='[{"disciplina": "...", "topico_edital": "...", ...}]')
        bruto = texto.strip() or None
    else:
        arquivo = st.file_uploader("Arquivo .json", type=["json"])
        if arquivo is not None: bruto = arquivo.read().decode("utf-8")
    ...
    if limpo.startswith("```"):
        limpo = limpo.split("```")[1]
        if limpo.lstrip().lower().startswith("json"): limpo = limpo.lstrip()[4:]

    try: dados = json.loads(limpo)
    except json.JSONDecodeError as e:
        st.error(f"JSON inválido: {e}")
        return

    if isinstance(dados, dict): dados = dados.get("questoes", [dados])
    if not isinstance(dados, list):
        st.error("O JSON precisa ser um array de questões.")
        return

    validas, rejeitadas, avisos = validar_lote(dados, TOPICOS_EDITAL, set(DISCIPLINAS_ESTUDO))

    c1, c2, c3 = st.columns(3)
    c1.metric("✅ Válidas", len(validas))
    c2.metric("❌ Rejeitadas", len(rejeitadas))
    c3.metric("⚠️ Avisos", len(avisos))

    if rejeitadas:
        with st.expander(f"Ver {len(rejeitadas)} questão(ões) rejeitada(s)"):
            for r in rejeitadas:
                st.markdown(f"- **#{r['indice'] + 1}** — {'; '.join(r['erros'])}  \n"
                            f"  <span style='color:#777;font-size:12px;'>{r.get('preview', '')}</span>", unsafe_allow_html=True)
    if avisos:
        with st.expander(f"Ver {len(avisos)} aviso(s)"):
            for a in avisos: st.markdown(f"- {a}")

    if not validas: return

    with st.expander("👀 Prévia das questões válidas"):
        st.dataframe(pd.DataFrame([{"Nível": f"N{q['nivel']}", "Disciplina": q["disciplina"], "Tópico": q["topico_edital"][:45] + "…", "Enunciado": q["enunciado"][:70] + "…", "Gab": q["gabarito"]} for q in validas]), use_container_width=True, hide_index=True)

    if st.button(f"💾 Importar {len(validas)} questão(ões)", use_container_width=True, type="primary"):
        try: sb.table("questoes").upsert(validas, on_conflict="hash", ignore_duplicates=True).execute()
        except Exception as e:
            st.error(f"Falha ao gravar no Supabase: {e}")
            return
        st.session_state.flow_banco_versao = st.session_state.get("flow_banco_versao", 0) + 1
        carregar_banco.clear()
        st.success(f"{len(validas)} questão(ões) processadas. Duplicatas foram ignoradas pelo hash.")

def _sub_painel(sb, TOPICOS_EDITAL):
    estados = carregar_estados(sb)
    df_resp = carregar_respostas(sb)

    if not estados:
        st.info("Nenhuma questão respondida ainda no modo Flow.")
        return

    banco = carregar_banco(sb, st.session_state.get("flow_banco_versao", 0))
    total_banco = len(banco)
    total_resp = len(df_resp)
    acc_geral = (df_resp["acertou"].mean() * 100) if total_resp else 0
    tempo_medio = (df_resp["tempo_seg"].mean()) if total_resp else 0

    st.markdown(f"""
    <div class="card-container">
        <div class="neon-card card-cyan"><div class="card-title">🗂️ QUESTÕES NO BANCO</div><div class="card-value">{total_banco}</div></div>
        <div class="neon-card card-emerald"><div class="card-title">✍️ RESPONDIDAS</div><div class="card-value">{total_resp}</div></div>
        <div class="neon-card card-violet"><div class="card-title">🎯 ACERTO NO FLOW</div><div class="card-value">{acc_geral:.0f}%</div></div>
        <div class="neon-card card-orange"><div class="card-title">⏱️ TEMPO MÉDIO</div><div class="card-value">{tempo_medio:.0f}s</div></div>
    </div>""", unsafe_allow_html=True)

    st.markdown("#### 📊 Nota R por subtópico (define o nível das próximas questões)")
    df_est = pd.DataFrame([{"Tópico": t[:55], "R": e["r"], "Nível": f"N{nivel_por_r(e['r'])}", "Disciplina": e.get("disciplina") or "—", "Questões": e["total_respondidas"]} for t, e in estados.items()]).sort_values("R")
    fig = px.bar(df_est.head(15), x="R", y="Tópico", orientation="h", color="R", color_continuous_scale="Teal", text="Nível", range_x=[0, 100])
    fig.update_traces(textposition="outside", textfont_color="white")
    fig.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font=dict(color="#E0E0E0"), coloraxis_showscale=False, xaxis_title="R (0-100)", yaxis_title="", yaxis=dict(autorange="reversed"), height=480)
    st.plotly_chart(fig, use_container_width=True)

    if total_resp >= 5:
        st.markdown("#### 🌊 Você esteve em flow? (média móvel de 8 questões)")
        df_resp = df_resp.reset_index(drop=True)
        df_resp["acerto_movel"] = df_resp["acertou"].astype(int).rolling(JANELA, min_periods=3).mean() * 100
        df_resp["n"] = df_resp.index + 1
        fig2 = px.line(df_resp, x="n", y="acerto_movel", markers=True)
        fig2.update_traces(line_color="#009CA6")
        fig2.add_hrect(y0=ALVO_ACERTO_MIN * 100, y1=ALVO_ACERTO_MAX * 100, fillcolor="#10B981", opacity=0.12, line_width=0, annotation_text="ZONA DE FLOW", annotation_position="top left")
        fig2.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font=dict(color="#E0E0E0"), xaxis_title="Questão nº", yaxis_title="% acerto (móvel)", yaxis=dict(range=[0, 105]))
        st.plotly_chart(fig2, use_container_width=True)

        st.markdown("#### ⏱️ Tempo vs. acerto")
        df_resp["situacao"] = df_resp.apply(lambda r: ("Acertou" if r["acertou"] else "Errou") + " — " + str(r["faixa_tempo"]), axis=1)
        fig3 = px.scatter(df_resp, x="tempo_seg", y="tempo_alvo_seg", color="situacao", hover_data=["topico_edital", "nivel"], size_max=12)
        fig3.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font=dict(color="#E0E0E0"), xaxis_title="Tempo gasto (s)", yaxis_title="Tempo-alvo (s)")
        st.plotly_chart(fig3, use_container_width=True)

    st.markdown("#### 📋 Estado completo")
    st.dataframe(df_est, use_container_width=True, hide_index=True)

    st.markdown("#### 🕳️ Buracos no banco (tópicos sem questão suficiente)")
    contagem = {}
    for q in banco: contagem[q["topico_edital"]] = contagem.get(q["topico_edital"], 0) + 1
    buracos = []
    for disc, topicos in TOPICOS_EDITAL.items():
        if disc not in PESOS_DISCIPLINA: continue
        for t in topicos:
            n = contagem.get(t, 0)
            if n < 5: buracos.append({"Disciplina": disc, "Tópico": t[:60], "Questões no banco": n})
    if buracos: st.dataframe(pd.DataFrame(buracos).sort_values("Questões no banco"), use_container_width=True, hide_index=True, height=240)
    else: st.success("✔️ Todos os tópicos têm pelo menos 5 questões no banco.")

def render_flow_tab(supabase, TOPICOS_EDITAL, DISCIPLINAS_ESTUDO):
    st.markdown("<h3 style='color:#009CA6;margin-bottom:6px;'>🌊 Modo Flow — Questões Adaptativas</h3>", unsafe_allow_html=True)
    st.markdown("<p style='color:#888;margin-top:-8px;font-size:14px;'>"
                "A dificuldade se ajusta pelo seu acerto <b>e</b> pelo tempo de cada questão. "
                "Alvo: 70–80% de acerto — nem tédio, nem ansiedade.</p>", unsafe_allow_html=True)
    sub_praticar, sub_importar, sub_painel = st.tabs(["🎯 Praticar", "📥 Importar Questões", "📊 Painel do Flow"])
    with sub_praticar: _sub_praticar(supabase, TOPICOS_EDITAL)
    with sub_importar: _sub_importar(supabase, TOPICOS_EDITAL, DISCIPLINAS_ESTUDO)
    with sub_painel: _sub_painel(supabase, TOPICOS_EDITAL)
...
# Abas expandidas incluindo o Modo Flow
tab_registro, tab_dash_treino, tab_dieta, tab_peso, tab_estudo, tab_dash_estudo, tab_flow, tab_cruzamento, tab_gerenciar = st.tabs([
    "📝 Treino", "📊 Dash Físico", "🥗 Dieta", "⚖️ Peso", "📚 Estudar", "📈 Dash Estudos", "🌊 Modo Flow", "🧬 Cruzamentos", "⚙️ Config"
])
...
# ==========================================
# ABA 7 (NOVA): MODO FLOW
# ==========================================
with tab_flow:
    render_flow_tab(supabase, TOPICOS_EDITAL, DISCIPLINAS_ESTUDO)
